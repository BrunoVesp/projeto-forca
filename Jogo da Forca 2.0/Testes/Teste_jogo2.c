...
//-->#include "Menu.c" --- Se ela não for declarada no menu(switch) vai dar problemas,Tenho que ver o porque[Acredito ser meu IDE]
...
void Jogo(){
	...
	int p_attenpts [0] = MAX_TENTATIVAS;
	int p_attenpts [1] = MAX_TENTATIVAS;
	int p_attenpts [2] = MAX_TENTATIVAS;
	int p_attenpts [3] = MAX_TENTATIVAS;
...
void jogos(){
	...
	while(tentativas < 0){
		...
		for(i = 0; i < Num_players; i++){
			for(j = 0; j < Num_players; i++){
				//-->Bloco de Jogo
				printf("Vez do: %s",NomePlayers[i][j]);
